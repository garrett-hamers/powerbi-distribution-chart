...
- Make **Home > Refresh > Schema and data** a required step in the sample `.pbix` procedure, before **File > Save As**. A PBIP caches no data, so Power BI Desktop opens the project reporting incomplete tables; saving straight from a freshly opened project produces a `.pbix` with empty tables, which fails AppSource review. Documented in `docs/partner-center-submission.md` section 4.1, the generated `samples/README.md`, and the `npm run audit:submission` operator output, together with the check that the refresh must complete without any credential prompt - a prompt would mean something external had entered the semantic model.
